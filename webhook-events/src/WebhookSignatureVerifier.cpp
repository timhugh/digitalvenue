...
namespace digitalvenue::webhook_events {
    ...
    std::string computeSignature(const std::string &payload,
        ...
        return {reinterpret_cast<char *>(hash.data()), hashLength};
    }

    WebhookSignatureVerifierResult WebhookSignatureVerifier::verify(
            const std::string &requestBody, const std::string &signature,
        ...
        auto computedSignature = digitalvenue::core::base64::encode(
                reinterpret_cast<const unsigned char *>(hash.c_str()), hash.size());
