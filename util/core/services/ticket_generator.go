...
type TicketGenerator struct {
	qrGenerator core2.QRCodeGenerator
}
...
func (t *TicketGenerator) GenerateTickets(ctx context.Context, order *core2.Order) error {
...
func buildTickets(ctx context.Context, order *core2.Order) ([]*core2.Ticket, error) {
	_, log := logger.FromContext(ctx)

	tickets := make([]*core2.Ticket, len(order.Items))
...
func buildTicket(order *core2.Order, item *core2.OrderItem) *core2.Ticket {
	return &core2.Ticket{
		ID:         item.ID,
